...
async function handler(req: NextRequest) {
  try {
    ...
    if (updated.user.email) {
      await sendNunnyStatusEmail(updated.user.email, updated.user.fullName, 'SUSPENDED')
    }

    return NextResponse.json({ message: 'Nunny suspended successfully', nunny: updated })
  } catch (error) {
    console.error('Error suspending nunny:', error)
